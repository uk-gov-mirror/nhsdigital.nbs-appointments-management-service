...
public class ReferenceNumberProviderTests
{
    ...
    {
        _siteStore.Setup(x => x.GetReferenceGroup("test")).ReturnsAsync(0);
        ...
        _siteStore.Verify(x => x.SaveReferenceGroup("test", 14), Times.Once());
    }

    [Fact]
    public async Task GetReferenceNumber_DoesNotAssignsRefGroup_WhenAlreadyAssigned()
    {
        _siteStore.Setup(x => x.GetReferenceGroup("test")).ReturnsAsync(14);

        await _sut.GetReferenceNumber("test");

        _referenceNumberDocumentStore.Verify(x => x.AssignReferenceGroup(), Times.Never);
        _siteStore.Verify(x => x.SaveReferenceGroup("test", It.IsAny<int>()), Times.Never);
    }
