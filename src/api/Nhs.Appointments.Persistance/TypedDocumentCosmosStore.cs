    ...
    where TDocument : TypedCosmosDocument, new()
{
    ...
    private async Task<IEnumerable<TSource>> IterateResults<TSource>(FeedIterator<TSource> queryFeed, string path, bool canExtractRequestCharge = true) // TODO: Remove this if we can always extract the RequestCharge from the Sql query.
    {
